@extends('layouts.autumnLayout', ['kzLink' => 'faculties/' . $item->slug_kz, 'ruLink' => 'faculties/' . $item->slug_ru, 'enLink' => 'faculties/' . $item->slug_en])
...
@section('content')
    <div class="container mx-auto px-2">
        ...
        <div class="lg:hidden mt-[60px] rounded-[15px] overflow-hidden shadow-md">
            <div>
                <div id="toggleStructureMenu" class="bg-autumn-main px-[25px] py-[15px]">
                ...
                <div id="structureMenu" class="bg-white px-[25px] py-[15px]">
                        ...
                        <div class="pl-[20px]">
                            @foreach($item->parent->children as $child)
                                <div>
                                    @if($child->id === $item->id)
                                        <span
                                            class="text-autumn-main font-semibold">{{ $item->getProperty('name') }}
                                                </span>
                                    @else
                    ...
                    @else
                        <div class="">
                            <div><span class="text-autumn-main font-semibold">{{ $item->getProperty('name') }}</span>
                            </div>
                            @if($item->children)
                                <div class="pl-[20px]">
                                    @foreach($item->children as $child)
                            ...
                            <div class="mt-[60px]">
                                ...
                                <div class="flex flex-wrap justify-between mt-[30px]">
                                    @foreach(json_decode($item->getProperty('staff')) as $member)
                                        <div
                                            class="border border-autumn-main w-full md:w-[48%] mb-[20px] p-[20px] rounded-[15px]">
                                            <div class="">
                                ...
                                @foreach(json_decode($item->getProperty('documents')) as $document)
                                    <div class="mb-[40px]">
                                        <a href="/{{ $document->path }}" target="_blank"
                                           class="hover:text-autumn-main font-semibold text-md transition-all duration-300">
                        ...
                        <div>
                            ...
                            @else
                                <div class="">
                                    <div>
                                        <span
                                            class="text-autumn-main font-semibold">{{ $item->getProperty('name') }}</span>
                                    </div>
                                    @if($item->children)
                                        <div class="pl-[20px]">
                                            @foreach($item->children as $child)
