    ...
    <div class="px-2 xl:container mx-auto">
        <div class="py-[44px] xl:py-[100px] flex flex-col xl:flex-row justify-center items-center">
            <div class="w-full xl:px-0 xl:w-[842px] xl:mr-[100px]">
                ...
                <div class="flex items-center mt-[24px] md:mt-[37px] text-[14px] sm:text-[16px] text-summer-heading">
                    <div class="flex items-center mr-[20px]">
                        <i class="fad fa-calendar-alt text-summer-secondary"></i><span
                            class="ml-[6px]">{{ $latestArticle->formatted_date }}</span>
                    </div>
                    <div class="flex items-center mr-[20px]">
                        <i class="fas fa-eye text-summer-secondary"></i><span
                            class="ml-[6px]">{{ $latestArticle->getProperty('views') }}</span>
                    </div>
                    <div class="flex items-center mr-[20px]">
                        <i class="fas fa-heart text-summer-secondary"></i><span
                            class="ml-[6px]">{{ $latestArticle->likes->count() }}</span>
                    </div>
                    <div class="flex items-center">
                        <i class="fas fa-comment text-summer-secondary"></i><span
                            class="ml-[6px]">{{ $latestArticle->comments->count() }}</span>
                    </div>
                </div>
                <div
                    class="text-[18px] md:text-[32px] mt-[24px] font-bold text-summer-heading">{{ $latestArticle->getProperty('title') }}</div>
                <div class="mt-[24px] flex">
                    <a href="{{ route('news.show', $latestArticle->getProperty('slug')) }}"
                       class="relative flex justify-between items-center bg-summer-main rounded-full overflow-hidden group transition-all duration-300 hover:shadow-lg hover:scale-105">
                        <div
                            class="text-[14px] lg:text-[18px] pl-[31px] font-semibold text-white relative z-10 transition-all duration-300">
                            {{ __('home.readArticle') }}
                        </div>
                        <div
                            class="flex items-center justify-center ml-[21px] w-[40px] md:w-[64px] h-[40px] md:h-[64px] bg-summer-extra text-white rounded-full relative z-10 transition-all duration-300 group-hover:bg-summer-extra group-hover:scale-110">
            ...
            <div class="w-full xl:w-auto mt-[44px] xl:mt-[0]">
                <div class="flex items-center justify-between mb-[24px]">
                    ...
                    <a href="{{ route('news.index') }}"
                       class="font-semibold text-[14px] xl:text-[18px] text-summer-main flex items-center group">
                    ...
                    <div class="flex items-center py-[24px] border-[#E2E2E2] border-t group">
                        ...
                        <div class="text-[12px] xs:text-[16px] font-semibold">
                            <div class="h-[53px] xs:h-auto">
                                <a href="{{ route('news.show', $item->getProperty('slug')) }}" class="transition-colors duration-300 group-hover:text-summer-main">
                                    {{ $item->getProperty('title') }}
                                </a>
                            </div>
                            <div class="flex items-center mt-[5px] xs:mt-[24px] text-[10px] xs:text-[14px] text-summer-secondary">
