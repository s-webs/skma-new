<div class="overflow-hidden" x-data="{
    ...
    <div class="h-auto xl:h-[840px] bg-winter-main relative">
        <img src="/assets/images/cliparts/winter_01.png" alt="" class="absolute z-[3] w-full h-full object-cover opacity-20">
                        ...
                        <div class="flex items-center">
                            <template x-for="(review, index) in reviews" :key="index">
                                <div class="h-[8px] mr-[8px] rounded-full"
                                     :class="{'bg-winter-main w-[30px]': currentIndex === index, 'bg-[#DBC5FF] w-[8px]': currentIndex !== index}"></div>
                            </template>
                        </div>
                        <div class="flex items-center">
                            <button @click="toggleSlideshow()"
                                    class="border flex justify-center items-center border-[#E2E2E2] w-[35px] xl:w-[64px] h-[35px] xl:h-[64px] rounded-full text-[10px] xl:text-[14px] text-white hover:text-winter-main mr-[12px] bg-winter-main hover:bg-winter-halftone transition-colors duration-300">
                                <i class="fas"
                                   :class="{'fa-pause': !isPlaying, 'fa-play': isPlaying}"></i>
                            </button>
                            <button @click="prevSlide()"
                                    class="border flex justify-center items-center border-[#E2E2E2] w-[35px] xl:w-[64px] h-[35px] xl:h-[64px] rounded-full text-[16px] xl:text-[30px] text-white hover:text-winter-main mr-[12px] bg-winter-main hover:bg-winter-halftone transition-colors duration-300">
                                <i class="fas fa-angle-left translate-y-[1px]"></i>
                            </button>
                            <button @click="nextSlide()"
                                    class="border flex justify-center items-center border-[#E2E2E2] w-[35px] xl:w-[64px] h-[35px] xl:h-[64px] rounded-full text-[16px] xl:text-[30px] text-white hover:text-winter-main bg-winter-main hover:bg-winter-halftone transition-colors duration-300">
