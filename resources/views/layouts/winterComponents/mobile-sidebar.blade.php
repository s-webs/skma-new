<div id="sidebarMobile"
     class="lg:hidden flex flex-col h-[100vh] w-[100%] fixed bg-winter-halftone z-20 -translate-x-full transition-transform duration-300">
    <div class="flex justify-end px-[16px] bg-winter-heading text-white py-[8px]">
        <button class="text-[24px]" id="closeMobileSidebar"><i class="fas fa-times"></i></button>
    </div>
    {{-- MobileSearch --}}
    <div class="mt-[16px] px-2" x-data="formSearch">
        <form @submit.prevent class="bg-white text-winter-secondary border border-winter-secondary rounded-full">
    ...
    <div class="px-2 mt-[16px]">
        <a href="{{ route('login') }}" class="block bg-winter-main rounded-full px-[24px] py-[8px] text-white font-semibold hover:bg-winter-extra transition-colors duration-300">Войти</a>
    </div>
    <div class="my-[24px] px-[16px]">
        <span class="block h-[1px] bg-winter-secondary"></span>
    </div>
    {{-- MobileMenu --}}
                ...
                @foreach ($menus as $menu)
                    <li class="relative">
                        @if ($menu->children->isNotEmpty())
                            <div
                                class="flex items-center justify-between mb-[16px] py-2 px-4 bg-winter-halftone hover:bg-winter-main text-winter-secondary hover:text-white border border-winter-secondary rounded-full transition-colors duration-300 cursor-pointer"
                    ...
                    <span>
                                ...
                                @foreach ($menu->children as $child)
                                    <li class="mb-[16px]">
                                        <a href="{{ $child->getProperty('link') }}"
                                           class="block py-2 px-[16px] bg-winter-halftone hover:bg-winter-main text-winter-secondary hover:text-white border border-winter-secondary rounded-full transition-colors duration-300">
                                            {!! $child->icon !!}<span
                        ...
                        @else
                            <a class="block mb-[16px] py-2 px-4 bg-winter-halftone hover:bg-winter-main text-winter-secondary hover:text-white border border-winter-secondary rounded-full transition-colors duration-300"
    ...
    {{-- /MobileMenu --}}
    {{-- MobileSidebarFooter --}}
    <div class="text-center bg-winter-heading py-[22px]">
