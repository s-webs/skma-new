<div id="sidebarMobile"
     class="lg:hidden flex flex-col h-[100vh] w-[100%] fixed bg-autumn-halftone z-20 -translate-x-full transition-transform duration-300">
    <div class="flex justify-end px-[16px] bg-autumn-heading text-white py-[8px]">
        <button class="text-[24px]" id="closeMobileSidebar"><i class="fas fa-times"></i></button>
    </div>
    {{-- MobileSearch --}}
    <div class="mt-[16px] px-2" x-data="formSearch">
        <form @submit.prevent class="bg-white text-autumn-secondary border border-autumn-secondary rounded-full">
    ...
    <div class="px-2 mt-[16px]">
        <a href="{{ route('login') }}" class="block bg-autumn-main rounded-full px-[24px] py-[8px] text-white font-semibold hover:bg-autumn-extra transition-colors duration-300">Войти</a>
    </div>
    <div class="my-[24px] px-[16px]">
        <span class="block h-[1px] bg-autumn-secondary"></span>
    </div>
    {{-- MobileMenu --}}
                ...
                @foreach ($menus as $menu)
                    <li class="relative">
                        @if ($menu->children->isNotEmpty())
                            <div
                                class="flex items-center justify-between mb-[16px] py-2 px-4 bg-autumn-halftone hover:bg-autumn-main text-autumn-secondary hover:text-white border border-autumn-secondary rounded-full transition-colors duration-300 cursor-pointer"
                    ...
                    <span>
                                ...
                                @foreach ($menu->children as $child)
                                    <li class="mb-[16px]">
                                        <a href="{{ $child->getProperty('link') }}"
                                           class="block py-2 px-[16px] bg-autumn-halftone hover:bg-autumn-main text-autumn-secondary hover:text-white border border-autumn-secondary rounded-full transition-colors duration-300">
                                            {!! $child->icon !!}<span
                        ...
                        @else
                            <a class="block mb-[16px] py-2 px-4 bg-autumn-halftone hover:bg-autumn-main text-autumn-secondary hover:text-white border border-autumn-secondary rounded-full transition-colors duration-300"
    ...
    {{-- /MobileMenu --}}
    {{-- MobileSidebarFooter --}}
    <div class="text-center bg-autumn-heading py-[22px]">
