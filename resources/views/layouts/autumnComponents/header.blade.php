    ...
    <div class="bg-white shadow-md relative z-[5]">
        <div class="container px-4 2xl:px-[16px] mx-auto flex items-center">
            <div class="w-[104px] relative">
                <a href="{{ route('home') }}"
                   class="flex items-center justify-center bg-autumn-main w-[74px] md:w-[104px] h-[70px] md:h-[100px] absolute z-10 top-[-29px] md:top-[-28px] lg:top-[-39px] left-1/2 -translate-x-1/2 lg:left-[0px] lg:translate-x-0 rounded-bl-2xl rounded-br-2xl shadow-md pb-[10px] pt-[5px] lg:pb-[20px] lg:pt-[8px]">
                    ...
                    @foreach ($menus as $menu)
                        @if ($menu->children->isNotEmpty())
                            <li class="relative mx-[22px] group">
                                <span class="hover:text-autumn-main transition-colors duration-300">{{ $menu->getProperty('name') }} <i
                                    ...
                                    @foreach ($menu->children as $child)
                                        <li class="my-[16px] whitespace-nowrap">
                                            <a href="{{ $child->getProperty('link') }}"
                                               class="px-[16px] py-[6px] block hover:bg-autumn-halftone hover:text-autumn-main transition-colors duration-300">{{ $child->getProperty('name') }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </li>
                        @else
                            <li class="mx-[22px]">
                                <a href="{{ $menu->getProperty('link') }}"
                                   class="hover:text-autumn-main transition-colors duration-300">{{ $menu->getProperty('name') }}</a>
                            </li>
                        @endif
                    @endforeach
                </ul>
            ...
            <div class="hidden lg:block">
                <button id="openSearchModal"
                    class="bg-autumn-main w-[44px] h-[44px] rounded-full text-white hover:bg-autumn-extra transition-colors duration-300">
                    <i class="fas fa-search"></i>
                </button>
            </div>
            <div class="ml-[44px] hidden lg:block">
                <a href="{{ route('login') }}"
                   class="bg-autumn-main rounded-full px-[24px] py-[12.5px] text-white font-semibold hover:bg-autumn-extra transition-colors duration-300">Войти</a>
            </div>
