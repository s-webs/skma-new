<div id="sidebarMobile"
     class="lg:hidden flex flex-col h-[100vh] w-[100%] fixed bg-summer-halftone z-20 -translate-x-full transition-transform duration-300">
    <div class="flex justify-end px-[16px] bg-summer-heading text-white py-[8px]">
        <button class="text-[24px]" id="closeMobileSidebar"><i class="fas fa-times"></i></button>
    </div>
    {{-- MobileSearch --}}
    <div class="mt-[16px] px-2" x-data="formSearch">
        <form @submit.prevent class="bg-white text-summer-secondary border border-summer-secondary rounded-full">
    ...
    <div class="px-2 mt-[16px]">
        <a href="{{ route('login') }}" class="block bg-summer-main rounded-full px-[24px] py-[8px] text-white font-semibold hover:bg-summer-extra transition-colors duration-300">Войти</a>
    </div>
    <div class="my-[24px] px-[16px]">
        <span class="block h-[1px] bg-summer-secondary"></span>
    </div>
    {{-- MobileMenu --}}
                ...
                @foreach ($menus as $menu)
                    <li class="relative">
                        @if ($menu->children->isNotEmpty())
                            <div
                                class="flex items-center justify-between mb-[16px] py-2 px-4 bg-summer-halftone hover:bg-summer-main text-summer-secondary hover:text-white border border-summer-secondary rounded-full transition-colors duration-300 cursor-pointer"
                    ...
                    <span>
                                ...
                                @foreach ($menu->children as $child)
                                    <li class="mb-[16px]">
                                        <a href="{{ $child->getProperty('link') }}"
                                           class="block py-2 px-[16px] bg-summer-halftone hover:bg-summer-main text-summer-secondary hover:text-white border border-summer-secondary rounded-full transition-colors duration-300">
                                            {!! $child->icon !!}<span
                        ...
                        @else
                            <a class="block mb-[16px] py-2 px-4 bg-summer-halftone hover:bg-summer-main text-summer-secondary hover:text-white border border-summer-secondary rounded-full transition-colors duration-300"
    ...
    {{-- /MobileMenu --}}
    {{-- MobileSidebarFooter --}}
    <div class="text-center bg-summer-heading py-[22px]">
